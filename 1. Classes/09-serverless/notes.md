...
> Notes extracted from [Alvaro Navas'notes](https://github.com/ziritrion/ml-zoomcamp/blob/main/notes/09_serverless.md)
...
RUN pip3 install https://github.com/alexeygrigorev/tflite-aws-lambda/blob/main/tflite/tflite_runtime-2.7.0-cp39-cp39-linux_x86_64.whl
...
Docker commands:
* `docker build -t clothing-model .` builds the image.
* `docker run -it --rm -p 8080:8080 clothing-model:latest` runs the image on port 8080.

Test locally the docker container:
...
# Tag the image (in this case the tag is model-001)
docker tag my-model-image:latest repository-uri:model-001

# Push the image
docker push repository-uri:model-001
```
