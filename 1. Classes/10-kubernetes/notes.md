> Extracted from [Alvaro Nava's notes](https://github.com/ziritrion/ml-zoomcamp/blob/main/notes/10_kubernetes.md)
...
>Note: in lesson 9 - serverless we used tflite for serving models in AWS Lambda. Tensorflow Serving is the actual tool intended for deploying models; we only used tflite with Lambda because Tensorflow-Serving is too big for it.
...
> -1 means that the dimension is variable. In our example, the input is a batch of images, so the first dimension is the batch size and can be any number.
...
docker run -it --rm ^
    -p 8500:8500 ^
    -v "%cd%\clothing-model:/models/clothing-model/1" ^
...
* `-v "$(pwd)/clothing-model:/models/clothing-model/1"`
    ...
    * Both folder paths need to be absolute, so relative paths like `"./clothing-model"` won't work. We can use a convenient trick with the `pwd` command to print our current folder: `$(pwd)` will be replaced with the current folder of the host machine.
...
* `-e MODEL_NAME="clothing-model"`: environment variable. The image makes use of an environment variable called `MODEL_NAME` which must match the name of our model.
...
We already explored how to upload a Docker image to ECR in Lesson 9. Below is a shortened explanation to create a repository and upload the images:
...
# Check the output and save the repository URI
set repository-uri=755803508038.dkr.ecr.eu-south-2.amazonaws.com/mlzoomcamp-images

# Login to the repository
aws ecr get-login-password --region eu-south-2 | docker login --username AWS --password-stdin %repository-uri%

# Tag the images
docker tag zoomcamp-10-model:xception-v4-001 %repository-uri%:zoomcamp-10-model-xception-v4-001
docker tag zoomcamp-10-gateway:002 %repository-uri%:zoomcamp-10-gateway-002

# Push the images
docker push %repository-uri%:zoomcamp-10-model-xception-v4-001
docker push %repository-uri%:zoomcamp-10-gateway-002
```
